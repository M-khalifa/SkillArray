...
Rule text comes from references/review-protocol.md ("All seats" and "Rebuttal instruction"
under Standard seat instructions, "Model
identity", "Evidence and findings", "Web verification", "Falsification pass") and
references/phase-3-scorecard.md ("Instruct it to:" list). A missing section is an error, never
a silently shorter brief.
`;

function parseArgs(argv) {
  const args = { mode: null, packet: null, seat: null, out: null, outputPath: null, repoOnly: false,
    peerView: null, auditDir: null, targetDir: null, claim: null, rebuttal: null };
  const flags = { '--mode': 'mode', '--packet': 'packet', '--seat': 'seat', '--out': 'out',
    '--output-path': 'outputPath', '--peer-view': 'peerView', '--audit-dir': 'auditDir',
    '--target-dir': 'targetDir', '--claim': 'claim', '--rebuttal': 'rebuttal' };
  ...
  const need = { phase1: ['packet', 'seat'], delta: ['peerView'], auditor: ['packet', 'auditDir', 'targetDir'],
...
// Lines strictly between a marker line starting with `from` and the first line starting with `to`.
function extractBetween(text, from, to, source) {
  const lines = text.replace(/\r\n/g, '\n').split('\n');
  const a = lines.findIndex((l) => l.startsWith(from));
  const b = a === -1 ? -1 : lines.findIndex((l, i) => i > a && l.startsWith(to));
  if (a === -1 || b === -1) throw new RelayError(`marker "${from}" ... "${to}" not found in ${source}`);
...
async function buildPhase1(args, protocol) {
  ...
  if (!args.repoOnly) parts.push(extractSection(protocol, '## Web verification'));
...
async function buildAuditor(args, protocol, scorecard) {
  ...
  const instructions = extractBetween(scorecard, 'Instruct it to:', 'State this limitation explicitly', 'phase-3-scorecard.md');
  return [
...
async function build(args) {
  ...
  else if (args.mode === 'auditor') parts = await buildAuditor(args, protocol, await read(path.join(REFS, 'phase-3-scorecard.md')));
...
export { parseArgs, extractSection, extractBetween, build, RelayError };
