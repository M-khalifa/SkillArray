...
## P1 — [BUG] scan's target-derived exemption walks `.git/` and ignored dirs, so a branch name silently whitelists a third-person vendor identity leak
Severity: HIGH
Basis: EXECUTED
...
## P2 — [BUG] vendor-token detection is substring-based, so ordinary English words hard-stop the exchange (`affable` → `fable`)
Severity: MEDIUM
Basis: EXECUTED
Evidence strength: REPRODUCED
Evidence:
...
## P3 — [BUG][FEEDBACK-O12] translate's mechanical evidence capture swallows the trailing `Suggested fix:` line (any post-fence prose) into findings.json `evidence`
Severity: HIGH
Basis: EXECUTED
Evidence strength: REPRODUCED
Evidence:
...
Suggested fix: blind-relabel.mjs `extractClaimBlocks`: when "Evidence:" is followed by a fence, end the evidence body at that fence's closing line. Capture a `Suggested fix:` line into its own `suggested_fix` field (see P14) and keep it out of `evidence`.

## P4 — [BUG] back-to-back Evidence fences leak their own delimiter lines into derived evidence
Severity: MEDIUM
Basis: EXECUTED
Evidence strength: REPRODUCED
Evidence:
...
## P5 — [BUG] translate rewrites every X<n>/Y<n> token in prose fields, fabricating real claim IDs that do not exist
Severity: MEDIUM
Basis: EXECUTED
Evidence strength: REPRODUCED
Evidence:
...
## P6 — [BUG][FEEDBACK-O3] stale Phase 2 `P<n>` labels survive into the Phase 3 X/Y files and nothing detects them
Severity: MEDIUM
Basis: EXECUTED
Evidence strength: REPRODUCED
Evidence:
...
## P7 — [DOC][FEEDBACK-G3] rebuttal entry heading level is unspecified; any `##`-level entry heading fails validate, and the error sends the file to the wrong seat
Severity: MEDIUM
Basis: EXECUTED
Evidence strength: REPRODUCED
Evidence:
...
## P8 — [IMPROVE][FEEDBACK-O2] no helper appends translated rebuttals; the P-to-real step is manual even though `relabel --from P` already does it
Severity: MEDIUM
Basis: EXECUTED
Evidence strength: REPRODUCED
Evidence:
...
~~~~
relabel keeps numbers (`\b${from}(\d+)\b` becomes `${to}$1`), so the P-to-real mapping is the identity on numbers. The docs never say so. The fenced `P71` is left alone, which is the same gap as P6. Orchestrators currently use sed or ad-hoc regex for this step.
Suggested fix: add `blind-relabel.mjs append-rebuttals --in <rebuttal.md> --onto <peer-findings.md> --from-seat <X> --peer <Y>`. It should run the P-to-peer relabel, fail on any fenced P-ID, normalize entry headings to `###` (P7), append the exact `## Rebuttals (from X) of Y claims` heading, and then run `validate`.

## P9 — [DOC][FEEDBACK-G11] the auditor is told to "read" phase3/, which also holds the coin-flip mapping and the real-letter-named half-relabeled tmp files
Severity: HIGH
...
## P10 — [DOC][FEEDBACK-O8] "no limit, omit --timeout" contradicts the 1800 s default in both dispatchers
Severity: MEDIUM
Basis: STATIC_TRACE
Evidence strength: DETERMINISTIC
Evidence:
...
## P11 — [DOC][FEEDBACK-O9] SKILL.md's required Codex choice list is stale against the catalog
Severity: LOW
Basis: STATIC_TRACE
Evidence strength: DETERMINISTIC
Evidence:
...
## P12 — [DOC] 1.5.0 was released without a version bump; the repository's own validator fails at HEAD
Severity: MEDIUM
Basis: EXECUTED
Evidence strength: REPRODUCED
Evidence:
...
## P13 — [BUG] build-manifest keys repeated `--phase1/--phase2/--verification` hashes by basename and silently drops collisions
Severity: LOW
Basis: EXECUTED
Evidence strength: REPRODUCED
Evidence:
...
## P14 — [DOC][FEEDBACK-O12] findings.json schema has no human-readable claim text or fix field; auditor extras pass through undocumented and get ID-rewritten
Severity: MEDIUM
Basis: STATIC_TRACE
Evidence strength: DETERMINISTIC
Evidence:
...
~~~~
The ID-rewriting of these extra keys is the defect shown in P5.
...
Suggested fix: review-protocol.md findings.json: add required `title` (one line) and optional `suggested_fix`. Add `Suggested fix:` to the reviewer schema. blind-relabel.mjs: derive `suggested_fix` from the origin blocks (P3). Either reject unknown keys, or list which keys are passed through and which are translated.

## P15 — [TOKENS][FEEDBACK-O15] preflight embeds full Tier 2 stdout; the protocol says to inline it into every seat brief
Severity: MEDIUM
Basis: EXECUTED
Evidence strength: REPRODUCED
Evidence:
...
## P16 — [TOKENS][FEEDBACK-O1] harness-seat and auditor outputs are hand-backs the orchestrator must re-emit verbatim; make "write your own file" the default
Severity: MEDIUM
Basis: SOURCE_CITATION
...
Suggested fix: phase-1/phase-2 docs: the harness seat writes `<run-dir>/phase1/A-findings.md` (Phase 2: `phase2/A-rebuttals.md`) and returns only counts, and the orchestrator runs `validate` on the file. For the auditor, keep the "pathless" rule (P9): give it a single write-only output path outside `audit-input/`, for example `<run-dir>/phase3-out/findings.audit.json`.

## P17 — [DOC][FEEDBACK-O14] "resume the Claude agent by its exact ID" gives no mechanism
Severity: LOW
Basis: SOURCE_CITATION
Evidence strength: DETERMINISTIC
Evidence:
...
## P18 — [IMPROVE][FEEDBACK-O4] non-git targets lose all tamper evidence (preflight Tier 1 and touchedFiles both null)
Severity: LOW
Basis: STATIC_TRACE
Evidence strength: DETERMINISTIC
Evidence:
...
## P19 — [IMPROVE][FEEDBACK-O5] no extraction step for .docx/.pdf targets; both seats depend on unreviewed ad-hoc conversion
Severity: LOW
Basis: SOURCE_CITATION
Evidence strength: SUPPORTED
Evidence:
...
## P20 — [DOC][FEEDBACK-O6] web cap unit is ambiguous for batched search tools
Severity: LOW
Basis: SOURCE_CITATION
Evidence strength: DETERMINISTIC
Evidence:
...
## P21 — [IMPROVE][FEEDBACK-O7] trust-boundary rule (never fetch target-embedded URLs) has no mechanical check
Severity: LOW
Basis: SOURCE_CITATION
Evidence strength: DETERMINISTIC
Evidence:
...
## P22 — [TOKENS][FEEDBACK-O10] no brief/prompt builder: Phase 1 brief, both delta briefs, and the auditor prompt are hand-written each run
Severity: MEDIUM
Basis: SOURCE_CITATION
Evidence strength: SUPPORTED
Evidence:
...
## P23 — [TOKENS][FEEDBACK-O11] the orchestrator must read ~112 KB of docs before dispatch, much of it runtime-conditional
Severity: LOW
Basis: EXECUTED
Evidence strength: SUPPORTED
Evidence:
...
## P24 — [IMPROVE][FEEDBACK-O13] no guidance on image size for document targets
Severity: LOW
Basis: SOURCE_CITATION
Evidence strength: SUPPORTED
Evidence:
...
## P25 — [DOC][FEEDBACK-G1] the Model identity rule covers prose but not paths or filenames, although scan flags seat-letter paths written in prose
Severity: LOW
Basis: STATIC_TRACE
Evidence strength: DETERMINISTIC
Evidence:
...
~~~~
Suggested fix: phase-1-independent-passes.md verbatim Model identity text: add "or in any path, directory, or filename you create or cite". Also use it in the build-brief template (P22).

## P26 — [DOC][FEEDBACK-G6] the delta brief for the harness seat is unspecified (file, verbatim vs condensed)
Severity: LOW
Basis: SOURCE_CITATION
Evidence strength: DETERMINISTIC
Evidence:
...
~~~~
Suggested fix: phase-2-cross-examination.md: require the harness seat's resume message to carry the scanned `peer-view-for-A.md` verbatim (or its path, if P16 is adopted), never a condensed version, because the scan only validated the exact bytes.

## P27 — [IMPROVE][FEEDBACK-G7] "orchestrator claims as C-IDs, may be wrong" pattern is not documented
Severity: LOW
Basis: SOURCE_CITATION
...
## P28 — [IMPROVE][FEEDBACK-G8] Severity has no rubric, so seats disagree on acceptance-risk vs code-risk claims
Severity: LOW
Basis: SOURCE_CITATION
Evidence strength: SUPPORTED
Evidence:
...
## P29 — [IMPROVE][FEEDBACK-G9] no data-privacy lens (PII / hardware identifiers in captures)
Severity: LOW
Basis: SOURCE_CITATION
Evidence strength: SUPPORTED
Evidence:
...
## P30 — [IMPROVE][FEEDBACK-G10] report does not require the reason behind the falsification count
Severity: LOW
Basis: SOURCE_CITATION
Evidence strength: SUPPORTED
Evidence:
...
~~~~
Suggested fix: review-protocol.md manifest: add `falsification.breakdown: {high_or_critical, disputed, conceded, unaddressed}` computed mechanically (with P8's parser), so "0 qualified" explains itself.

## P31 — [IMPROVE][FEEDBACK-G12] Phase 3 double relabel is four hand-substituted commands with a known naming trap and an expected nonzero exit
Severity: MEDIUM
Basis: SOURCE_CITATION
Evidence strength: DETERMINISTIC
Evidence:
...
Suggested fix: add "prefer grep and line-range reads over whole-file reads of files >500 lines" to the brief template (P22). For redaction, send only the scan's flagged lines. The seat returns (or, with P16, rewrites in place) just those lines, and the scan re-runs on the whole file.

## P33 — [IMPROVE] Phase 2 appends rebuttals into phase1/*.md in place, so the pure Phase 1 bytes are never preserved or hashed
Severity: LOW
Basis: STATIC_TRACE
Evidence strength: DETERMINISTIC
Evidence:
...
- Tests: preflight.json Tier 2 records `node --test` exit 0 and `validate-package.mjs` exit 0 (not re-run). The repo-level `scripts/validate-repository.mjs` fails (P12).
- codex-dispatch.mjs: arg parsing, `--search` placed before `exec` on fresh and resume runs, session-identity fail-closed, touchedFiles porcelain -z parsing, and every result.json writer carries `webAccess`. No defect found beyond P10 and P18.
...
- FEEDBACK-O1..O15 and G1..G13: all others hold and are raised above. O1 and G2 are one claim (P16). O2 is P8. O3 is P6. O12 is split into P3 (evidence pollution) and P14 (schema).
