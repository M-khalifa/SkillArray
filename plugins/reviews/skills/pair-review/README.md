...
Two distinct Claude models review independently, then collaborate or challenge each other's findings.
...
`~/.claude/skills/pair-review/` for personal use, or
`.claude/skills/pair-review/` in a project for a shared installation.
Keep the folder name `pair-review`; the helper derives its identity from it.
...
/pair-review setup
/pair-review config
/pair-review -- review the changes in src/client.py
/pair-review reset
```

Setup asks for both models and optional effort before the first review.
Run setup again or say "change pair-review's second reviewer to MODEL" to update
...
Tests are offline. They verify configuration persistence and isolation.
